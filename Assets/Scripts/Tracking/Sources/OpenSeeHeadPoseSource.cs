...
namespace Belen.Tracking.Sources
{
    ...
    public class OpenSeeHeadPoseSource : MonoBehaviour, IHeadPoseSource
    {
        ...
        [Tooltip("Estimate Z distance from 2D face size (bounding box) instead of relying on tracker Z.")]
        public bool estimateZFromFaceSize = false;
        [Tooltip("Meters at neutral face size (used as baseline for depth estimation)")]
        public float neutralDepthMeters = 0.6f;
        [Tooltip("Face size in pixels at neutral distance (auto-calibrate)")]
        public float neutralFacePixels = 0f;
        [Tooltip("Blend: 0 = tracker Z only, 1 = face-size Z only")]
        [Range(0f,1f)] public float zBlend = 1.0f;
        [Tooltip("Clamp estimated Z in meters [min,max]")]
        public Vector2 zClamp = new Vector2(0.2f, 2.5f);
        [Tooltip("Smoothing time for Z estimation (seconds)")]
        ...
        private void Update()
        {
            ...
            // Optional Z estimation from face size
