...
{
    // Receives 2D face rectangle over UDP and maps to head pose without rotation.
    // Supported formats per datagram:
    // 1) JSON normalized: {"cx":0..1,"cy":0..1,"size":0..1}
    // 2) CSV normalized: cx,cy,size
    // Optional JSON fields: {"w":imageWidth,"h":imageHeight,"x":px,"y":py,"wbox":pxWidth,"hbox":pxHeight}
    // If pixel fields are present, they are converted to normalized internally.
    public class UdpFaceBoxReceiver : MonoBehaviour, IHeadPoseSource
    {
        [Header("Network")]
        ...
        public float neutralSizeNormalized = 0.25f; // typical face box fraction
