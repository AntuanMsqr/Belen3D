...
namespace Belen.Tracking.Sources
{
    ...
    // Supported formats (single datagram per sample):
    // 1) JSON: {"pos":[x,y,z],"rot":[pitch,yaw,roll],"ts":seconds}
    // 2) CSV: x,y,z,pitch,yaw,roll,ts
    public class UdpHeadPoseReceiver : MonoBehaviour, IHeadPoseSource
    {
        [Header("Network")]
        ...
        [Tooltip("Apply scale to incoming position (e.g., meters).")]
        public float positionScale = 1.0f;

        [Tooltip("Swap Y/Z for sources using Z-up.")]
        public bool swapYZ = false;

        [Tooltip("Negate Z (useful if forward axis is inverted).")]
        ...
        {
            // Very lightweight parsing to avoid JSON libs; expects keys pos, rot, ts
            pose = default;
            try
            {
